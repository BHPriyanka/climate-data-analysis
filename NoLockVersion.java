 ...
 * 		g. refer NoLockThread.java for thread details 
 *  	h. logs the execution time for ten executions
 */
public class NoLockVersion {
	public final static Logger logger = Logger.getLogger(NoLockVersion.class.getName());
	...
	public void divideRecordsEqually(List<String> records){
		...
		// create 4 Threads and adds to the arraylist
		List<NoLockThread> fourThreads = new ArrayList<NoLockThread>();
		int low;
		int high;
        for (int x = 0; x < cores; x++) {
            ...
            fourThreads.add(new NoLockThread("Thread " + x, records, low, high, stationAvgTemp));
        }      
	...
	/*
	 * NO_LOCK_version: Method to execute the basic functionality divideRecordsEqually of NoLockVersion class
	 ...
	 */
	public void NO_LOCK_version(List<String> records){
		...
		for(int i=0;i<10;i++){
			...
			if(i == 0){
				OutputWriter oW = new OutputWriter(stationAvgTemp, "NoLockVersionOutput");
			   	oW.writeResults();
			}
	...
	public static void main(String[] args){
		...
		noLock.NO_LOCK_version(load.records);	
	}
}
