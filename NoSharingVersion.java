 ...
 * 		f. refer NoShareThreadThread.java for thread details 
 *  	g. logs the execution time for ten executions
 */
public class NoSharingVersion {
	public final static Logger logger = Logger.getLogger(NoSharingVersion.class.getName());
	...
	public void divideRecordsEqually(List<String> records){
		...
		List<NoShareThread> fourThreads = new ArrayList<NoShareThread>();
		int low;
		int high;
        for (int x = 0; x < cores; x++) {
        	...
        	// Creating NoShareThread thread
            fourThreads.add(new NoShareThread("Thread " + x, records, low, high));
        }      
	...
	/*
	 * NO_SHARING_version: Method to execute the basic functionality divideRecordsEqually of NoSharingVersion class
	 ...
	 */
	public void NO_SHARING_version(List<String> records){
		...
		for(int i=0;i<10;i++){
		    ...
		    if(i == 0){
				OutputWriter oW = new OutputWriter(stationAvgTempMap, "NoSharingVersionOutput");
			   	oW.writeResults();
			};
	...
	public static void main(String[] args){
		...
		noSharing.NO_SHARING_version(load.records);
	}
}
