...
/*
 * CoarseLockThreadFibonacci extends Thread class and computes Average TMAX temperatures for each stationID 
 ...
 */
public class CoarseLockThreadFibonacci extends Thread {
	public final static Logger logger = Logger.getLogger(CoarseLockThreadFibonacci.class.getName());
	 ...
	 */
	CoarseLockThreadFibonacci(String threadname, List<String> records, int low, int high, HashMap<String, StationAvgTempEntry> stationAvgTemp) {
		name = threadname;
	...
	public void getStationAllTemperatures(){
		for(String s : this.tmax_records){
			...
			if(stationAvgTemp.containsKey(values[0])){
				//creating lock over entire shared data structure stationAvgTemp along with the delay being added
				...
				//creating lock over entire shared data structure stationAvgTemp along with a delay being added
				synchronized(stationAvgTemp){
